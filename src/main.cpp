...
#if defined(NDEBUG)
# error "Riotoken cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1f5bfec25f3f55c869b274e8f51fca4f6a277202827645f1a42f2cea78d6c572");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Riotoken: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Riotoken Signed Message:\n";
...
{
    // Riotoken: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 210000); // Riotoken: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 14 * 24 * 60 * 60; // Riotoken: 3.5 days
static const int64 nTargetSpacing = 10 * 60; // Riotoken: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Riotoken: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Riotoken: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x97b68304492a17367c6d440edd503efd04daea33e22ac26f0675ed1827a33d45");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "USA TODAY 11/Jan/2022 The past seven years have been Earth's hottest on record";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047451d2e97e9e748972a1e7ff657352232ef84175a5c00da85906c2595706788f660293354acf52d4989225b7caeeb2e3abcaf11fec41169ef15c95af4d200e83") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1642285011;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084791471;

        if (fTestNet)
        {
            block.nTime    = 1642284962;
            block.nNonce   = 386021711;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xdd8db0625c1e526ce4f55da714de3d4074202b9b954b14d605d1d4ef345ce8b9"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf5, 0xc1, 0xb0, 0xd0 }; // Riotoken: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RiotokenMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RiotokenMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RiotokenMiner(CWallet *pwallet)
{
    printf("RiotokenMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("riotoken-miner");
    ...
    try { loop {
        ...
        printf("Running RiotokenMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RiotokenMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RiotokenMiner, pwallet));
}
